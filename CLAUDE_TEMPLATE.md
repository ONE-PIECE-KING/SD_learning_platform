...
> **適用規模**: 4-6 人團隊 | **整合週期**: 3 天 | **分支策略**: Git Flow
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Day 1 (契約日)          Day 2-3 (開發日)      Day 3 (整合)  │
│  ┌─────────────┐        ┌─────────────┐      ┌─────────────┐│
│  │ 定義契約    │        │ 平行開發     │      │ Merge      ││
│  │ Review     │   →    │ 各自模組     │  →   │ 整合測試    ││
│  │ 簽核       │        │ 本地測試     │      │ 驗證契約    ││
...
|------|-------|--------|----------|
| `backend/core` | [Backend Lead] | [TL] | 核心業務邏輯 |
| `backend/api` | [Backend Lead] | [Backend Dev] | API 端點 |
| `frontend/core` | [Frontend Lead] | [TL] | 前端核心架構 |
| `frontend/features/*` | [Frontend Dev] | [Frontend Lead] | 功能模組 |
| `contracts/*` | [TL] | [Both Leads] | 契約定義 |

**所有權規則**:
- ✅ Owner 可直接修改自己的模組
- ⚠️ 非 Owner 修改需 PR + Owner Review
- 🔒 契約變更需所有依賴方同意
...
| 契約類型 | 用途 | 範本位置 |
|---------|------|----------|
| **API Contract** | 前後端 API 定義 | `CONTRACT_TEMPLATES/api_contract.yaml` |
| **TypeScript Interface** | 前端模組間型別 | `CONTRACT_TEMPLATES/typescript_interface.ts` |
| **Python Protocol** | 後端模組間抽象 | `CONTRACT_TEMPLATES/python_protocol.py` |
...
main (production)
  ...
  └── develop (integration)
        ...
        ├── feature/john-backend-user-api
        └── feature/jane-frontend-user-list
```

**命名規則**: `feature/<owner>-<module>-<feature>`

### 📅 3天整合週期

#### Day 1: 契約定義日
```
09:00  Sync Meeting (15-30min)
10:00  契約定義/更新
14:00  契約 Review
16:00  契約簽核 → 狀態變更為 approved
```
...
```
14:00  提交 PR
15:00  依序 Merge (基礎層 → 服務層 → API層 → UI層)
17:00  整合測試驗證
```
...
✅ 允許                           ❌ 禁止
─────────────────────────────────────────────────────────
透過契約定義的 Interface          直接 import 內部實作
透過 API Contract 通訊            繞過 API 直接存取資料
透過事件機制解耦                  跨模組共用非契約型別
```

### 📋 協作 SOP 參考

詳細操作步驟請參考: **`COLLABORATION_SOP.md`**
...
- **絕不**在契約未簽核前開始跨模組開發 → Day 1 必須完成契約
- **絕不**跳過測試的 Merge → CI 必須通過
- **絕不**引入未經核准的第三方依賴 → 新依賴需 TL 核准
