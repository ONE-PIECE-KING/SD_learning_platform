...
import { GraduationCap, Move, Bell, Shield, CreditCard, ChevronRight, Video } from "lucide-react";
...
export default function MemberSettings() {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {/* Become Teacher CTA - Only show for students */}
      {!isTeacher && (
      ...
      {/* Consultation Toggle - Only show for teachers */}
      {isTeacher && (
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <Video className="h-5 w-5 text-muted-foreground" />
              <CardTitle>一對一諮詢功能</CardTitle>
            </div>
            <CardDescription>設定是否要開放諮詢服務給學生</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <Label>開放諮詢服務</Label>
                <p className="text-sm text-muted-foreground">學生可以預約你的一對一諮詢時段</p>
              </div>
              <Switch defaultChecked />
            </div>
            <div className="flex items-center justify-between">
              <div>
                <Label>諮詢費用 (每小時)</Label>
                <p className="text-sm text-muted-foreground">NT$ 1,500</p>
              </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {(isTeacher 
              ? ["課程上架", "統計分析", "老師聯絡簿", "一對一諮詢", "個人簡介", "資源分享", "設定"]
              : ["我的課程", "購課記錄", "一對一諮詢", "個人簡介", "資源分享", "設定"]
            ).map((item, i) => (
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {isTeacher ? "當學生提交作業或提問時通知我" : "當課程有新單元時通知我"}
              </p>
            </div>
            <Switch defaultChecked />
          </div>
      ...
      <Card>
        <CardContent className="p-0">
          <Link to="/member/settings/security" className="flex items-center justify-between p-4 hover:bg-muted/50 transition-colors">
            <div className="flex items-center gap-3">
          ...
          <Link to="/member/settings/payment" className="flex items-center justify-between p-4 hover:bg-muted/50 transition-colors border-t border-border">
            <div className="flex items-center gap-3">
